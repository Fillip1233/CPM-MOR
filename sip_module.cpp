...
SIPResult SIPcore3_cpp(const SparseMatrix<double>& G,
...
PYBIND11_MODULE(sip_cpp, m) {
    m.doc() = "SIP algorithm C++ implementation";
    
    py::class_<SIPResult>(m, "SIPResult")
        .def_readwrite("G_hat", &SIPResult::G_hat)
        .def_readwrite("C_hat", &SIPResult::C_hat)
        .def_readwrite("E_hat", &SIPResult::E_hat);
    
    m.def("SIPcore3_cpp", &SIPcore3_cpp, 
          "SIP core algorithm in C++",
          py::arg("G"), py::arg("C"), py::arg("E"), 
          py::arg("ports"), py::arg("threshold") = 2.0);
}