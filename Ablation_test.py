...
if __name__ == "__main__":
    ...
    parser.add_argument("--draw_type", type= int, default= 3)
    ...
    res = res_ann(hidden_size=args.hidden_size, d_num=101).to(device)
    res.load_state_dict(torch.load(data_path + '/res_ann.pth'))
    

    alpha = alpha_ann(hidden_size=args.hidden_size, d_num=101).to(device)
    alpha.load_state_dict(torch.load(data_path + '/alpha_ann.pth'))
    ...
    with torch.no_grad():
        ypred1 = res.forward_h(x_test, yl_test)
        ypred2 = alpha.forward_h(x_test, yl_test)
        ypred3 = mynn.forward_h(x_test, yl_test)
    ...
    for i in range(100):
        ...
        vmin = torch.min((yte[i]-ypred1[i]).abs())
        vmax = torch.max((yte[i]-ypred1[i]).abs())

        im1 = axs[0].imshow((yte[i].cpu()-ypred1[i].cpu()).abs(), cmap='viridis', interpolation='nearest', vmin = vmin, vmax = vmax, aspect='auto')
        axs[0].set_title('Fixed scalar predict error')

        axs[1].imshow((yte[i].cpu()-ypred2[i].cpu()).abs(), cmap='viridis', interpolation ='nearest', vmin = vmin, vmax = vmax, aspect='auto')
        axs[1].set_title('Learnable scalar predict error')

        im2 = axs[2].imshow((yte[i].cpu()-ypred3[i].cpu()).abs(), cmap = 'viridis', interpolation='nearest', vmin = vmin, vmax = vmax, aspect='auto')
        axs[2].set_title('MF-TCA predict error')

        # cbar1 = fig.colorbar(im1, cax=cbar_ax1)
        cbar2 = fig.colorbar(im1, cax=cbar_ax2,location='right', fraction=0.02, pad=0.02)
        cbar2.formatter.set_powerlimits((0, 0))  # 强制使用科学计数法
        cbar2.update_ticks()
        fig.suptitle("Ablation stduy on ibmpg3t with three difference MF structure", fontsize=14)
        # plt.tight_layout()
    ...
    #         plt.title("ibmpg4t total port response", fontsize=14)
