...
if __name__ == '__main__':
    ...
    parser.add_argument("--circuit_size", type=int, default= 2)
    parser.add_argument("--threshold", type=float, default= 1.0)
    parser.add_argument("--svd_type", type=int, default= 1)
    parser.add_argument("--load", type=int, default= 0)
    parser.add_argument("--generate", type=int, default= 1)
    args = parser.parse_args()
    save_path = os.path.join(sys.path[0], 'train_data/2t/sim_100_port2000_multiper_svdB')
    ...
    if args.load == 0:
        if args.svd_type == 0:
            # svd mor
            # 设置load path可以直接加载数据
            left, right, B_r, O_r = svdmor(G, B, B, threshold = args.threshold, load_path="F:\zhenjie\code\CPM-MOR\code_try")
        else:
            # my svd mor
