...
def generate_DeMOR_data(G, C, B, y, args, savepath, seed):
    ...
    for i in range(RGA.shape[0]):
        ...
        if 0 not in indices:
            indices = np.insert(indices, 0, 0)  # 在位置 0 插入 0
        dominant_inputs.append(indices)
    print(dominant_inputs[3000].shape)
    
    # IS, VS = generate_udiff(args.port_num, args.circuit, seed = seed)
    IS, VS = generate_uover(args.port_num, args.circuit, seed = seed)
    x0 = np.zeros((C.shape[0], 1))
    ...
    #     for i, (port_idx, yy_mor) in enumerate(results):
    #         y_mor_combined[port_idx, :] = yy_mor.squeeze()
    y_mor_combined = process_output(3000, idx_list=dominant_inputs[3000], x0=x0, IS=IS, VS=VS, m=m)[1]
...
if __name__ == "__main__":
    ...
    if args.generate:
        ...
        for i in range(args.data_num):
            ...
            t3 = time.time()
            y_low = generate_DeMOR_data(G, C, B, y, args, save_path, seed=99)
            t4 = time.time() - t3
            logging.info(f"DeMOR data generation time for {i}th input: {t4:.4f} seconds")

            recording = {'rmse':[], 'nrmse':[], 'r2':[],'mae':[], 'relative_error':[]}
            metrics = calculate_metrix(y_test = np.abs(y[3000,:]), y_mean_pre = np.abs(y_low[0,:]))
            ...
            record.to_csv(data_path + '/res_DeMOR_over.csv', index = False)

            y_save = y_low
            np.save(f'DeMOR{args.circuit}_over.npy', y_save)
    ...
    plt.plot(time1, y_low[0,:], color='black', linestyle='-.', marker='*', label='DeMOR', markevery = 35, markersize=6, linewidth=1.5)
    plt.plot(time1, y[3000,:], color='blue', linestyle='-.', marker='*', label='GroundTruth', markevery = 25, markersize=6, linewidth=1.5)
    plt.legend(fontsize=12)
    plt.title(f"single-Port output response", fontsize=14)
    plt.xlabel("Time (s)", fontsize=12)
    plt.ylabel("Response result (V)", fontsize=12)
    plt.grid()
    plt.tight_layout()
    plt.savefig(f'demor{args.circuit}_over.pdf', dpi=300)
    plt.clf()
