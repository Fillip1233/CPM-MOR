...
class tensor_lstm(nn.Module):
    def __init__(self, data_shape_list, hidden_size, d_num=100, num_layers=1):
        super(tensor_lstm, self).__init__()
    ...
    def forward(self, u):
        ...
        u = u.permute(0, 2, 1)
        rnn_out, (h_n, c_n) = self.rnn(u)  # LSTM 多了一个 c_n
...
def train_tensor_lstm(tensor_lstm ,data_manager, lr, epoch, normal = False):
    
    optimizer = torch.optim.Adam(tensor_lstm.parameters(), lr = lr)
    ...
    for i in range(epoch):
        ...
        optimizer.zero_grad()
        y_res = y_h - tensor_lstm.Tensor_linear_list[0](y_l)
        u_pred = tensor_lstm(x_h)
