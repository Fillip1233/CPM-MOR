...
def prepare_data_mix(data_path, train_data_num=100):

    x1 = np.load(data_path+'/mf_inall.npy')
    x2 = np.load(data_path+'_multiper'+'/mf_inall.npy')
    x1 = torch.tensor(x1, dtype=torch.float32)
    x2 = torch.tensor(x2, dtype=torch.float32)
    x_trainl = torch.cat((x1[:train_data_num,:], x2[:train_data_num,:]), dim = 0)
    x_trainh = x_trainl
    ...
    y_l = torch.cat((yl1[:train_data_num,:], yl2[:train_data_num,:]), dim = 0)
    ...
    y_h = torch.cat((yh1[:train_data_num, :], yh2[:train_data_num,:]), dim = 0)

    time = np.load(data_path+'/mf_time.npy')

    x_test = torch.cat((x1[train_data_num:, :], x2[train_data_num:, :]), dim = 0)
    y_test = torch.cat((yh1[train_data_num:, :], yh2[train_data_num:, :]), dim = 0)
    yl_test = torch.cat((yl1[train_data_num:, :], yl2[train_data_num:, :]), dim = 0)
