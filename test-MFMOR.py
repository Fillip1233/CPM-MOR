...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="test MF-MOR")
    ...
    parser.add_argument("--exp_marker", type= str, default= "sparse")
    ...
    if args.test_over:
        ...
        x_test, y_test, yl_test, time1, pr = load_over_data_mfmor(data_path1, circuit=args.cir)
    ...
    if args.test_over:
        ...
        record.to_csv(data_path + '/test_{}_res_MFMOR.csv'.format(args.module_name), index = False)
    ...
    if args.draw_type == 2:
        ...
        for i in range(100):
            ...
            yy_low = torch.zeros((y_low.shape[1])).to(device)
            yy_low = y_low[i, :]
            np.save(f'./MSIP_BDSM/Exp_res/MF-MOR-over.npy', yy_1.cpu().numpy())
            if pr is not None:
                ppr = torch.zeros((pr1.shape[1])).to(device)
                ppr = pr1[i, :]
            ...
            plt.grid()
            plt.tight_layout()
            plt.savefig(f'./MSIP_BDSM/Exp_res/{args.cir}t_2per/MFMOR-over.pdf', dpi=300)
            plt.clf()
            break
    if args.draw_type == 3:
        pass